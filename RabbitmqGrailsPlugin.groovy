        ...
        if(!connectionFactoryUsername || !connectionFactoryPassword || !connectionFactoryHostname) {
            ...
            if(queuesConfig) {
                ...
                queueBuilder.bindings?.each { binding ->
                    ...
                    Closure convertBind = { rule ->
                        rule.split(",")
                            .findAll { it.trim() != ""  }
                    }


                    def rules = binding.rule.contains(",") ? convertBind(binding.rule)  : [binding.rule]

                    rules.each { rule ->
                        println "binding routeKey ${rule} for queue ${binding.queue} on exchange ${binding.exchange}"
                        "grails.rabbit.binding.${binding.exchange}.${binding.queue}.${rule}"(Binding, binding.queue, QUEUE, binding.exchange, rule, binding.arguments )
                    }                    
                }
            }
